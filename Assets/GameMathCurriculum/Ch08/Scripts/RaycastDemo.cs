...
public class RaycastDemo : MonoBehaviour
{
    ...
    {
        if (!selectedObject || !hit.collider.CompareTag(groundTag)) return;

        var placePos = hit.point + Vector3.up * placeOffsetY;
        var newGo = Instantiate(selectedObject, placePos, selectedObject.transform.rotation);
        newGo.tag = "Untagged";
