...
public class Assignment_UIPopup : MonoBehaviour
{
    ...
    private void UpdateAnimation()
    {
        ...
        {
            easedValue = EaseOutBounce(t);
            popupPanel.localScale = Vector3.Lerp(Vector3.zero, Vector3.one * 2f, easedValue);
            canvasGroup.alpha = Mathf.Clamp(easedValue, 0f, 1f);

            if (t >= 1f)
            {
                currentState = PopupState.Visible;
                popupPanel.localScale = Vector3.one * 2f;
        ...
        {
            easedValue = EaseOutBounce(t);
            popupPanel.localScale = Vector3.Lerp(Vector3.one * 2f, Vector3.zero, easedValue);
            canvasGroup.alpha = Mathf.Clamp(easedValue, 1f, 0f);
    ...
    private void UpdateUI()
    {
        ...
        stateText.text =
            $"<b>[UI Popup Animation]</b>\n" +
            $"상태: {stateLabel}\n" +
            $"진행도: {progress:P0}\n" +
            $"Scale: {popupPanel.localScale.x:F2}\n" +
            $"Alpha: {canvasGroup.alpha:F2}\n" +
            $"\n<color=grey><b>[ Space ] 토글  [ Esc ] 강제종료</b></color>";
    }
}
