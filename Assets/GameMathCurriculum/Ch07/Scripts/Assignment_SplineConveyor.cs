...
public class Assignment_SplineConveyor : MonoBehaviour
{
    ...
    private void Update()
    {
        // Catmull-Rom 스플라인 경로 위에 여러 박스를 일정 간격으로 배치하고, 4장에서 배운 AnimationCurve 로 구간별 속도를 조절하며 순환 이동시킨다.
        if (waypoints == null || waypoints.Length < 2 || boxes == null || boxes.Length == 0) return;

        globalT = (Time.time % cycleDuration) / cycleDuration;
        currentSpeedMultiplier = speedCurve.Evaluate(globalT);

        for (int i = 0; i < boxes.Length; i++)
        {
            float boxT = Mathf.Repeat(globalT + (float)i / boxes.Length, 1f);
            var boxPos = EvaluateSpline(waypoints, boxT);
            boxes[i].position = boxPos;
        }
