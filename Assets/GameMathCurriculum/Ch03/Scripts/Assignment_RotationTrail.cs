...
public class Assignment_RotationTrail : MonoBehaviour
{
    ...
    private void Update()
    {
        var trailAngle = Time.time * rotationSpeed;
        var trailMatrix = Matrix4x4.TRS(
            transform.position,
            Quaternion.Euler(0f, 0f, autoRotate ? trailAngle : rotationAngle),
            Vector3.one
        );
        Vector3 tipPos = trailMatrix.MultiplyPoint(new Vector3(0f, armLength, 0f));
        lastTipPos = tipPos;
