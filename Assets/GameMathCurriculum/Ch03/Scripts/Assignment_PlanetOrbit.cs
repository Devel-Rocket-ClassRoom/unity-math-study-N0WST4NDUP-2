...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private void Update()
    {
        var planetAngle = Time.time * planetOrbitSpeed;
        var planetOrbitMatrix = Matrix4x4.TRS(
            orbitCenter,
            Quaternion.Euler(0f, planetAngle, 0f),
            Vector3.one
        );
        transform.position = planetOrbitMatrix.MultiplyPoint(new(planetOrbitRadius, 1f, 0f));

        var satelliteAngle = Time.time * satelliteOrbitSpeed;
        var satelliteOrbitMatrix = Matrix4x4.TRS(
            transform.position,
            Quaternion.Euler(0f, satelliteAngle, 0f),
            Vector3.one
        );
        satellite.position = satelliteOrbitMatrix.MultiplyPoint(new(satelliteOrbitRadius, 0f, 0f));
