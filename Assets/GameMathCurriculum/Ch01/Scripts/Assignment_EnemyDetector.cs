...
public class Assignment_EnemyDetector : MonoBehaviour
{
    ...
    {
        float halfFOV = detectionFOV / 2f;
        float threshold = Mathf.Cos(halfFOV * Mathf.Deg2Rad);  // cos(60°) = 0.5

        Vector3 forward = transform.forward;
        Vector3 toEnemy = (enemy.position - transform.position).normalized;
        float dot = Vector3.Dot(forward, toEnemy);

        if (dot >= threshold && (transform.position - enemy.position).sqrMagnitude <= detectionRange * detectionRange)
        {
            return true;
        }
        return false;
